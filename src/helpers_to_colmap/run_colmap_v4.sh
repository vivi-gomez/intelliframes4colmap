...
Info "Backend seleccionado: COLMAP 4.x"
...
echo "[1/5] Extrayendo características..."
FE_ARGS=(
    ...
    --FeatureExtraction.use_gpu "$FE_USE_GPU"
)

if [[ "$USE_CPU" -eq 0 ]]; then
    FE_ARGS+=(--FeatureExtraction.gpu_index "$GPU_INDEX")
fi
...
if [[ "$USE_CPU" -eq 0 ]]; then
    MATCH_ARGS+=(--FeatureMatching.gpu_index "$GPU_INDEX")
fi
...
if [[ "$USE_CPU" -eq 0 && "$CASPAR_AVAILABLE" == "yes" ]]; then
    Info "Usando backend CASPAR para bundle adjustment"
    MAPPER_ARGS+=(
    ...
    Info "Usando backend por defecto del binario"
fi
